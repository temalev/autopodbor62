...
### ☐ 2. Уникальные meta‑теги на главной и /dkp

Сейчас `index.vue` и `dkp.vue` не вызывают `useHead()`, поэтому подхватывают глобальные title/description из `nuxt.config.ts` — общие для всех страниц.

Что сделать:
- Добавить `useHead()` в [app/pages/index.vue](app/pages/index.vue) с уникальным title и description, сфокусированным на главных коммерческих запросах ("автоподбор Рязань", "проверка авто Рязань").
- Добавить `useHead()` в [app/pages/dkp.vue](app/pages/dkp.vue) (онлайн‑конструктор ДКП — отдельный запросный куст).
