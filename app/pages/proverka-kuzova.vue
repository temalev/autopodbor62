...
const relatedServices = [
  {
    to: '/proverka',
    title: 'Разовая проверка авто',
    description: 'Полная диагностика автомобиля перед покупкой — отчёт с 150+ фото.',
  },
  {
    to: '/podbor',
    title: 'Подбор под ключ',
    description: 'Не хотите рисковать с битыми авто? Подберём гарантированно чистую машину.',
  },
  {
    to: '/vykup',
    title: 'Выкуп и комиссионная продажа',
    description: 'Поможем продать автомобиль после ремонта по справедливой цене.',
  },
]
...
async function submitLead(e: Event) {
  ...
  const res = await sendLead({ name, phone, source: 'proverka-kuzova' })
  if (res.ok) {
...
const highlights = [
  ...
  },
  {
    ...
    text: 'Проверяем равномерность зазоров, наличие дефектов: подтёков, шагрени, включений. Сверяем объём выполненных работ с нормо-часами.',
  },
  {
...
const steps = [
  ...
  { num: '03', title: 'Сверка с нормо-часами', text: 'Сопоставляем фактический объём и качество работ с тем, что указано в заказ-наряде. Выявляем расхождения и экономию сервиса.' },
...
const checklist = [
  ...
  'Контроль равномерности зазоров между элементами',
  ...
  <div class="kuzov-page">
    <div class="kuzov-page__hero">
      ...
      <div class="kuzov-page__hero-content">
        ...
        <h1 class="kuzov-page__title">Приёмка автомобиля после кузовного ремонта в Рязани</h1>
        <p class="kuzov-page__lead">Убедитесь, что сервис сделал всё, за что взял деньги. Принимаем автомобиль после ремонта, кузовных работ или покупки у дилера. Замеряем ЛКП, контролируем качество, сверяем с нормо-часами и фиксируем каждое нарушение.</p>
      </div>
    </div>
    ...
    <section class="kuzov-page__section">
      <div class="kuzov-page__container">
        ...
        <p class="kuzov-page__intro">Не поверхностный осмотр, а профессиональный контроль качества с профессиональным оборудованием и фиксацией всех нарушений.</p>
      ...
      <div class="kuzov-page__container">
        ...
        <div class="kuzov-page__cta">
          ...
          <form class="kuzov-page__form" @submit.prevent="submitLead">
            ...
            <p v-if="leadStatus === 'sent'" class="kuzov-page__form-hint">Заявка отправлена. Перезвоним в ближайшее время.</p>
            <p v-else-if="leadStatus === 'error'" class="kuzov-page__form-hint kuzov-page__form-hint--error">Не удалось отправить заявку. {{ leadError }}</p>
          </form>
        </div>
      </div>
    </section>
...
.kuzov-page__form-hint {
  margin: 10px 0 0;
  font-size: 14px;
  color: rgba(17, 17, 17, 0.8);
}

.kuzov-page__form-hint--error {
  color: #b42318;
}
</style>
