...
useHead({
  title: 'Автоподбор в Рязани под ключ — проверка, выкуп, учёт',
  meta: [
    {
      name: 'description',
      content: 'Автоподбор в Рязани под ключ: подбор, проверка перед покупкой, выкуп и комиссионная продажа, регистрация в ГИБДД, импорт из Кореи, Китая и Японии.',
    },
    { property: 'og:url', content: `${SITE_URL}/` },
    { property: 'og:title', content: 'Автоподбор в Рязани под ключ — проверка, выкуп, регистрация авто | Автоподбор 62' },
    {
  ...
  script: [
    jsonLdScript(
      serviceJsonLd({
        ...
        description:
          'Полный цикл подбора автомобиля в Рязани: анализ рынка, поиск, проверка, сопровождение сделки и регистрация в ГИБДД.',
        url: `${SITE_URL}/`,
        serviceType: 'Автоподбор и проверка автомобиля',
        // Без image в разметку подставлялся квадратный логотип 1684×1684 —
        // для превью услуги он не годится.
        image: HOME_OG_IMAGE,
      }),
      'ld-home-service',
    ),
    jsonLdScript(
      breadcrumbJsonLd([{ name: 'Главная', url: `${SITE_URL}/` }]),
      'ld-home-breadcrumbs',
    ),
    jsonLdScript(
      faqPageJsonLd([
        ...
        {
          question: 'Что если я уже нашёл автомобиль сам?',
          answer:
            'В этом случае мы проведём разовую проверку автомобиля: общий осмотр, проверка документов и истории, силовой конструкции кузова, системы освещения, замер ЛКП, проверка салона и систем безопасности, шин и тормозов, компьютерная диагностика. По результатам — полный отчёт с 150+ фото и видео и заключение эксперта.',
        },
        {
          question: 'Даёте ли вы какие-то гарантии?',
          answer:
            'Да. Гарантия юридической чистоты автомобиля, гарантия актуальности информации о его состоянии, а также гарантия выкупа подобранного автомобиля в течение 90 дней.',
        },
        {
          question: 'Почему требуется предоплата?',
          answer:
            'Предоплата — это оплата нашей работы по поиску, аналитике и организации проверок автомобилей. Она гарантирует вовлечённость обеих сторон и компенсирует расходы в случае, если клиент решает прекратить подбор.',
        },
        {
          question: 'Вы помогаете с регистрацией автомобиля в ГИБДД?',
          answer:
            'Да. Мы можем полностью взять регистрацию на себя: подача документов, осмотр, получение номеров. В рамках подбора под ключ по РФ и импорта автомобилей из других стран регистрация в ГИБДД входит в стоимость услуги.',
        },
        {
          question: 'Сколько стоят ваши услуги?',
          answer:
            'Стоимость зависит от ценовой категории автомобиля и выбранной услуги. Цены прозрачные и фиксируются заранее — без скрытых доплат. Точную стоимость мы всегда озвучиваем до начала работ.',
        },
      ]),
      'ld-home-faq',
    ),
...
const services = [
  ...
  },
  {
    ...
    description: 'Привезём автомобиль из Кореи, Китая или Японии под ключ — подбор, проверка, таможня, доставка в Рязань и постановка на учёт.',
    features: [
      { icon: 'public', text: 'Подбор и проверка автомобиля в Корее, Китае или Японии' },
...
<template>
  <div class="page">
    ...
    <section id="korea" class="page__section page__section--korea">
      ...
      <div class="page__korea-content">
        ...
        <h2 class="page__korea-title">Покупка автомобиля в Корее, Китае и Японии под ключ</h2>
      ...
      <div class="page__afterservice-wrap">
        <div class="page__afterservice-content">
          <div class="page__afterservice-left">
            ...
            <a href="tel:+79209570017" class="page__afterservice-btn">Записаться на ТО</a>
          </div>
  ...
  <!-- На мобильном текст скрыт, остаётся только иконка — имя ссылке даёт aria-label. -->
  <a :href="SITE_PHONE_HREF" class="page__fixed-call" :aria-label="`Позвонить: ${SITE_PHONE_DISPLAY}`">
